...
(function () {
  ...
  for (var key in init) {
    ...
    init[key].rgb = (function () {
      var colorObject = getSelectionColor(sel, key);
      if (!colorObject) {
        // 色が？のとき
        init[key].colored = null;
        return defaultColor[key];
      } else {
        var colorArray = colorObject2Array(colorObject);
  ...
  var w = new Window(
    "dialog{\
      text:'HSL',target:'fill',shiftOn:false,\
      ...
      rnd:Button{alignment:'fill',text:'端数処理'},\
      css:Group{orientation:'column',alignment:'fill',alignChildren:'fill',\
        hsl:EditText{properties:{readonly:true}},\
        rgb:EditText{properties:{readonly:true}},\
        hex:EditText{properties:{readonly:true}},\
      },\
      adb:Button{alignment:'fill',text:'スウォッチに追加'},\
  ...
  // 塗り・線ボタンの再生成
  w.v.recreateButton = function (button) {
  ...
  // カラーピッカーの処理
  w.v.callColorPicker = function () {
    var target = w[w.target];
    var currentColor = target.rgb;
    var pickColor = app.showColorPicker(rgbArray2Object(currentColor));
    if (compareColorObject(currentColor, pickColor)) return;
    w[w.target].rgb = colorObject2Array(pickColor);
    w[w.target].hsl = rgb2hsl(target.rgb);
    w.o.update();
    w.css.update();
    this.redraw();
  };
  ...
  w.v.f.onClick = function () {
    if (w.target === this.name) this.parent.callColorPicker();
    else this.parent.recreateButton(this);
  };
  ...
  w.css.update = function () {
    this.hsl.text = hsl2css(w[w.target].hsl);
    this.rgb.text = rgb2css(w[w.target].rgb);
    this.hex.text = rgb2hex(w[w.target].rgb);
  };
  ...
  // 丸めるボタン
  w.rnd.onClick = function () {
    w[w.target].hsl = roundArray(w[w.target].hsl);
    w.o.update();
    w.css.update();
    w.v.redraw();
  };
  ...
  // スウォッチに追加ボタン
  w.adb.onClick = function () {
    var color = w[w.target];
    var hsl = color.hsl;
    var newSwatch = doc.swatches.add();
    newSwatch.name = "H=" + hsl[0] + "° S=" + hsl[1] + "% L=" + hsl[2] + "%";
    newSwatch.name = w.css.hsl.text;
    newSwatch.color = rgb2colorObj(color.rgb);
    w.swatch.init();
  };

  // グループアイコン作成
  w.swatch.addGroupIcon = function () {
    var y = this.children[this.children.length - 1].bounds[3] + 3;
    var a = 13;
    var icon = this.add("staticText", [0, y, 0 + 13, y + 13]);
  ...
  // ボタン作成
  w.swatch.addButton = function (swatch) {
    var x = (y = 0);
    if (this.children.length > 0) {
      var prevBounds = this.children[this.children.length - 1].bounds;
      x = prevBounds[2] + 1;
      y = prevBounds[1];
      if (x + 13 > this.size.width) {
        x = 0;
        y = prevBounds[3] + 1;
      }
    }
    var button = this.add("button", [x, y, x + 13, y + 13]);
    // ボタンのクリックイベント
    ...
    button.onDraw = function () {
      ...
      if (this.active) {
        g.newPath();
        g.rectPath(0.5, 0.5, aside - 1, aside - 1);
        g.strokePath(g.newPen(g.BrushType.SOLID_COLOR, rgb4brush(255), 1));
        g.newPath();
        g.rectPath(1.5, 1.5, aside - 3, aside - 3);
        g.strokePath(g.newPen(g.BrushType.SOLID_COLOR, rgb4brush(0), 1));
      }
    };
  ...
  w.swatch.init = function () {
    ...
    this.size[1] = (function () {
      ...
      return y + (swatches.length - 2) * 3 + 4;
    })();
    w.layout.resize();
    ...
    for (var i = 0; i < swatches.length; i++) {
      ...
      for (var j = 0; j < list.length; j++) {
        ...
        button.rgb = colorObject2Array(swatch.color);
      }
    }
  };
  ...
  // 色変更
  colorise(sel, w.fill.rgb, w.stroke.rgb);
})();
...
function getSelectionColor(items, target, sourceColor) {
  var prop = target + "Color";
  for (var i = 0; i < items.length; i++) {
    var item = items[i];
    var type = item.typename;
    if (type === "GroupItem") {
      var result = getSelectionColor(item.pageItems, target, sourceColor);
...
function colorObject2Array(color) {
  var type = color.typename;
  if (type === "RGBColor") {
    return roundArray([color.red, color.green, color.blue]);
  } else if (type === "NoColor") {
    return null;
  } else if (type === "GrayColor") {
    var cvpo = Math.round((1 - color.gray / 100) * 255);
...
function colorise(arr, fill, stroke) {
  for (var i = 0; i < arr.length; i++) {
    var current = arr[i];
    var type = current.typename;
    if (type === "Groupitem") {
      colorise(current.pageItems, fill, stroke);
      continue;
    }
    ...
    if (fill === null) {
      ...
      current.filled = true;
      current.fillColor = rgbArray2Object(fill);
    }
    if (stroke === null) {
      current.stroked = false;
    } else {
      current.stroked = true;
      current.strokeColor = rgbArray2Object(stroke);
    }
  }
}
